    ...
    <section className="flex flex-col md:flex-row justify-between items-center py-8 sm:py-12 md:py-20 px-4 sm:px-6 md:px-8">
      <div className="flex flex-col gap-4 sm:gap-6 md:gap-8 w-full md:w-[40%]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[#4338CA] text-base sm:text-lg md:text-xl leading-7 font-medium dark:text-[#B7AAFF]">{header?.name}</span>
        </div>

        <div>
          <h1 className="text-lightHeading text-3xl sm:text-4xl md:text-5xl lg:text-7xl leading-tight md:leading-none font-bold dark:text-darkHeading">{header?.titleLine1}</h1>
          <h1 className="text-lightHeading text-3xl sm:text-4xl md:text-5xl lg:text-7xl leading-tight md:leading-none font-bold dark:text-darkHeading">{header?.titleLine2}</h1>
        </div>

        <p className="text-lightDescription text-base sm:text-lg leading-7 font-medium break-words dark:text-white max-w-[90%] sm:max-w-[85%] md:max-w-full">{header?.description}</p>
